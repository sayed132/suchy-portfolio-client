...
const PortfolioDetail = () => {
  ...
  const {data: portfolio = [], refetch} = useQuery({
    ...
    queryFn:  async ()=>{
      const res = await fetch(`https://suchy-portfolio-server.onrender.com/portfolio/${id}`);
      ...
      <div className="grid grid-cols-7 gap-5 lg:mx-8 p-5 items-center">
        <div className="col-span-full lg:col-span-2 text-justify">
          <div className="mb-3">
            ...
            <h3 className="font-bold text-xl text-blue-600">{portfolio?.name}</h3>
          ...
          <div >
            ...
            <p className="font-bold text-sm mb-3 mt-1 text-green-600">{portfolio?.time}</p>
            <h2 className="font-bold text-lg">Links</h2>
            <p className="font-bold text-sm mb-3 mt-1">
              <Link to={portfolio?.liveLink} target="_blank" className="pr-3 cursor-pointer hover:text-blue-700 duration-700">Live Link</Link>
              <Link className="cursor-pointer hover:text-blue-700 duration-700 pr-3" to={portfolio?.WorkLink} target="_blank">Work Link</Link>
              <Link className="cursor-pointer hover:text-blue-700 duration-700" to={portfolio?.WorkLink2} target="_blank">Work Link 2</Link>
            </p>
          </div>
        </div>
        ...
        <div className="col-span-full lg:col-span-5">
          <div className="flex flex-row-reverse justify-between">
            ...
            >
              <img
                className="transform duration-1000 ease-linear w-full  rounded-md"
                src={portfolio?.image}
